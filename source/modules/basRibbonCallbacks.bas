Attribute VB_Name = "basRibbonCallbacks"
Option Compare Database

'################################################################
'#                                                              #
'#      Created with / Erstellt mit:                            #
'#      IDBE RibbonCreator 2021                                 #
'#      Version: 1.1001                                         #
'#                                                              #
'#      (c) 2007-2022 IDBE Avenius                              #
'#                                                              #
'#      http://www.RibbonCreator2021.com                        #
'#      http://www.ribboncreator2019.com                        #
'#      http://www.ribboncreator2016.com                        #
'#      http://www.ribboncreator2010.com                        #
'#      http://www.ribboncreator.com                            #
'#      http://www.accessribon.com                              #
'#      http://www.avenius.com                                  #
'#                                                              #
'#      You may send change requests or report errors to:       #
'#      Aenderungswuensche oder Fehler bitte an:                #
'#                                                              #
'#      mailto://info@ribboncreator.com                         #
'#                                                              #
'################################################################


' Globals:


Global Const strAppPicturePath As String = "Pics"             ' The pictures/icons are available in the following directory
                                                              ' below the database directory
                                                              ' %Databasepath%\Pics
                                                              ' Die Bilder / Icons liegen in folgendem Verzeichnis
                                                              ' unterhalb des Datenbankverzeichnises
                                                              ' %Datenbankpfad%\Pics
                                                              
Global Const bolUseDynamicPicturePath As Boolean = False      ' The images should be loaded from this directory
                                                              ' and not from the directory in the Ribbon XML.
                                                              ' Die Bilder sollen aus diesem Verzeichnis geladen werden
                                                              ' und nicht ueber die Verzeichnisangabe im Ribbon XML.

                                                              ' These values are used in the function "GetImages"
                                                              ' Diese Werte werden in der Funktion "GetImages" verwendet.

                                                              
Public gobjRibbon As IRibbonUI

Public bolEnabled As Boolean    ' Used in Callback "getEnabled"
                                ' Further informations in Callback "getEnabled"
                                ' Fuer Callback "getEnabled"
                                ' Genauere Informationen in Callback "getEnabled".
                               
Public bolVisible As Boolean    ' Used in Callback "getVisible"
                                ' More information in Callback "getVisible
                                ' Fuer Callback "getVisible"
                                ' Further informations in Callback "getVisible

' For Sample Callback "GetContent"
' Fuer Beispiel Callback "GetContent"
Public Type ItemsVal
    ID As String
    label As String
    imageMso As String
End Type


' Callbacks:

Sub OnRibbonLoad(ribbon As IRibbonUI)
    ' Callbackname in XML File "onLoad"

    Set gobjRibbon = ribbon
End Sub

Sub LoadImages(control, ByRef Image)
    ' Callbackname in XML File "loadImage"

    ' Loads an image with transparency to the ribbon
    ' Modul basGDIPlus is required
    ' Laed ein Bild mit Transparenz in das Ribbon
    ' Modul basGDIPlus wird dafuer benoetigt
    
    Dim strImage        As String
    Dim strPicture      As String
    
    strImage = CStr(control)
    strPicture = getPic(strImage)
    
    If strImage <> "" Then
        If bolUsePicturesFromTable = True Then
            If strPicture <> "" Then
                Set Image = getIconFromTable(strPicture)
            Else
                Set Image = Nothing
            End If
        Else
            Set Image = LoadPictureGDIP(strImage)
        End If
    Else
        Set Image = Nothing
    End If
    
End Sub

Sub GetImages(control As IRibbonControl, ByRef Image)
    ' Callbackname in XML File "getImages"
    
    ' Loads an image with transparency to the ribbon
    ' Modul basGDIPlus is required
    ' Laed ein Bild mit Transparenz in das Ribbon
    ' Modul basGDIPlus wird dafuer benoetigt
    
    Dim strPicturePath  As String
    Dim strPicture      As String
    
    strPicture = getTheValue(control.Tag, "CustomPicture")
    
    If bolUsePicturesFromTable = True Then
        Set Image = getIconFromTable(strPicture)
    Else
        If bolUseDynamicPicturePath = True Then
            strPicturePath = getAppPath & strAppPicturePath & "\"
        Else
            strPicturePath = getTheValue(control.Tag, "CustomPicturePath")
        End If
        Set Image = LoadPictureGDIP(strPicturePath & strPicture)
    End If

End Sub

Sub GetEnabled(control As IRibbonControl, ByRef enabled)
    ' Callbackname in XML File "getEnabled"
    
    ' To set the property "enabled" to a Ribbon Control
    ' For further information see: http://www.accessribbon.de/en/index.php?Downloads:12
    ' Setzen der Enabled Eigenschaft eines Ribbon Controls
    ' Weitere Informationen: http://www.accessribbon.de/index.php?Downloads:12
    
    Select Case control.ID
        
        Case "btn_database"
            enabled = DbProcedures.GetPermission(control.ID).Read

        Case "btn_setup"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_customfield"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_translation"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_user"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_role"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_mailqueue"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_error"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_billing"
            enabled = DbProcedures.GetPermission(control.ID).Read
        
        Case "btn_laboratory"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_department"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_workplace"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_instrument"
           enabled = DbProcedures.GetPermission(control.ID).Read
          
        Case "btn_customer"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_contact"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_manufacturer"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_supplier"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_smpmatrix"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_smptype"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_smppoint"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_smpcondition"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_smppreservation"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_container"
            enabled = DbProcedures.GetPermission(control.ID).Read
        
        Case "btn_storage"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_instype"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_technique"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_priority"
            enabled = DbProcedures.GetPermission(control.ID).Read
        
        Case "btn_desktop"
            enabled = DbProcedures.GetPermission(control.ID).Read
        
        Case "btn_info"
            enabled = True
            
        Case "btn_certificate"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_qualification"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_method"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_analysis"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_profile"
            enabled = DbProcedures.GetPermission(control.ID).Read
        
        Case "btn_template"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_workflow"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_mdi"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_request"
            enabled = DbProcedures.GetPermission(control.ID).Read
        
        Case "btn_measurement"
            enabled = DbProcedures.GetPermission(control.ID).Read
        
        Case "btn_batch"
            enabled = DbProcedures.GetPermission(control.ID).Read
        
        Case "btn_material"
            enabled = DbProcedures.GetPermission(control.ID).Read
        
        Case "btn_service"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_formulation"
            enabled = DbProcedures.GetPermission(control.ID).Read
            
        Case "btn_project"
            enabled = DbProcedures.GetPermission(control.ID).Read

        Case Else
            enabled = True

    End Select

End Sub

Sub GetVisible(control As IRibbonControl, ByRef visible)
    ' Callbackname in XML File "getVisible"
    
    ' To set the property "visible" to a Ribbon Control
    ' For further information see: http://www.accessribbon.de/en/index.php?Downloads:12
    ' Setzen der Visible Eigenschaft eines Ribbon Controls
    ' Weitere Informationen: http://www.accessribbon.de/index.php?Downloads:12

    Select Case control.ID
        
        Case "btn_database"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_server
            visible = True
        Case "grp_server"
            ' Group:    grp_server
            ' In Tab:   tab_setup
            visible = True
        Case "btn_setup"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_config
            visible = True
        Case "btn_customfield"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_config
            visible = True
        Case "btn_translation"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_config
            visible = True
        Case "btn_user"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_config
            visible = True
        Case "btn_role"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_config
            visible = True
        Case "btn_mailqueue"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_config
            visible = True
        Case "btn_error"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_config
            visible = True
        Case "grp_config"
            ' Group:    grp_config
            ' In Tab:   tab_setup
            visible = True
        Case "tab_setup"
            ' Tab:   tab_setup
            visible = True
        Case "btn_laboratory"
            ' Button
            ' In Tab:   tab_infrastructure
            ' In Group: grp_instance
            visible = True
        Case "grp_instance"
            ' Group:    grp_instance
            ' In Tab:   tab_infrastructure
            visible = True
        Case "btn_department"
            ' Button
            ' In Tab:   tab_infrastructure
            ' In Group: grp_institution
            visible = True
        Case "btn_workplace"
            ' Button
            ' In Tab:   tab_infrastructure
            ' In Group: grp_institution
            visible = True
        Case "btn_instrument"
            ' Button
            ' In Tab:   tab_infrastructure
            ' In Group: grp_institution
            visible = True
        Case "grp_institution"
            ' Group:    grp_institution
            ' In Tab:   tab_infrastructure
            visible = True
        Case "tab_infrastructure"
            ' Tab:   tab_infrastructure
            visible = True
        Case "btn_customer"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_address
            visible = True
        Case "btn_contact"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_address
            visible = True
        Case "btn_manufacturer"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_address
            visible = True
        Case "btn_supplier"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_address
            visible = True
        Case "grp_address"
            ' Group:    grp_address
            ' In Tab:   tab_catalogs
            visible = True
        Case "btn_smpmatrix"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            visible = True
        Case "btn_smptype"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            visible = True
        Case "btn_smppoint"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            visible = True
        Case "btn_smpcondition"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            visible = True
        Case "btn_smppreservation"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            visible = True
        Case "btn_container"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            visible = True
        Case "grp_sampling"
            ' Group:    grp_sampling
            ' In Tab:   tab_catalogs
            visible = True
        Case "btn_instype"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_other
            visible = True
        Case "btn_priority"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_other
            visible = True
        Case "grp_other"
            ' Group:    grp_other
            ' In Tab:   tab_catalogs
            visible = True
        Case "tab_catalogs"
            ' Tab:   tab_catalogs
            visible = True
        Case "btn_certificate"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_certificate
            visible = True
        Case "btn_qualification"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_certificate
            visible = True
        Case "grp_certificate"
            ' Group:    grp_certificate
            ' In Tab:   tab_testing
            visible = True
        Case "btn_method"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_testing
            visible = True
        Case "btn_analysis"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_testing
            visible = True
        Case "btn_profile"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_testing
            visible = True
        Case "btn_template"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_testing
            visible = True
        Case "grp_testing"
            ' Group:    grp_testing
            ' In Tab:   tab_testing
            visible = True
        Case "btn_workflow"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_order
            visible = True
        Case "btn_mdi"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_order
            visible = True
        Case "btn_request"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_order
            visible = True
        Case "grp_order"
            ' Group:    grp_order
            ' In Tab:   tab_testing
            visible = True
        Case "tab_testing"
            ' Tab:   tab_testing
            visible = True

        Case Else
            visible = True

    End Select

End Sub

Sub GetLabel(control As IRibbonControl, ByRef label)
    ' Callbackname in XML File "getLabel"
    ' To set the property "label" to a Ribbon Control
    
    Select Case control.ID
        
        Case "tab_setup"
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        
        Case "tab_infrastructure"
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        
        Case "tab_catalogs"
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        
        Case "tab_testing"
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        
        Case "tab_tools"
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        
        Case "btn_database"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_server
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_server"
            ' Group:    grp_server
            ' In Tab:   tab_setup
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_setup"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_config
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_customfield"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_config
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_translation"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_config
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_user"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_config
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_role"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_config
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_log"
            ' Group:    grp_server
            ' In Tab:   tab_setup
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_mailqueue"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_config
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_error"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_config
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_billing"
            ' Group:    grp_server
            ' In Tab:   tab_setup
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_billing"
            ' Button
            ' In Tab:   tab_setup
            ' In Group: grp_config
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_config"
            ' Group:    grp_config
            ' In Tab:   tab_setup
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "tab_setup"
            ' Tab:   tab_setup
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_laboratory"
            ' Button
            ' In Tab:   tab_infrastructure
            ' In Group: grp_instance
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_instance"
            ' Group:    grp_instance
            ' In Tab:   tab_infrastructure
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_department"
            ' Button
            ' In Tab:   tab_infrastructure
            ' In Group: grp_institution
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_workplace"
            ' Button
            ' In Tab:   tab_infrastructure
            ' In Group: grp_institution
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_instrument"
            ' Button
            ' In Tab:   tab_infrastructure
            ' In Group: grp_institution
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_institution"
            ' Group:    grp_institution
            ' In Tab:   tab_infrastructure
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "tab_infrastructure"
            ' Tab:   tab_infrastructure
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_customer"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_address
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_contact"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_address
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_manufacturer"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_address
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_supplier"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_address
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_address"
            ' Group:    grp_address
            ' In Tab:   tab_catalogs
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_smpmatrix"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_smptype"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_smppoint"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_smpcondition"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_smppreservation"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_container"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_storage"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_sampling"
            ' Group:    grp_sampling
            ' In Tab:   tab_catalogs
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_instype"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_other
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_technique"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_other
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_priority"
            ' Button
            ' In Tab:   tab_catalogs
            ' In Group: grp_other
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_other"
            ' Group:    grp_other
            ' In Tab:   tab_catalogs
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "tab_catalogs"
            ' Tab:   tab_catalogs
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_certificate"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_certificate
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_qualification"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_certificate
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_desktop"
            ' Group:    grp_certificate
            ' In Tab:   tab_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_desktop"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_info"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_certificate"
            ' Group:    grp_certificate
            ' In Tab:   tab_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_method"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_analysis"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_profile"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_template"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_testing"
            ' Group:    grp_testing
            ' In Tab:   tab_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_workflow"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_order
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_mdi"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_order
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_request"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_order
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
         Case "btn_measurement"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_order
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_batch"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_order
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_chemistry"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_order
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_material"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_order
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_service"
            ' Button
            ' In Tab:   tab_testing
            ' In Group: grp_order
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_order"
            ' Group:    grp_order
            ' In Tab:   tab_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_info"
            ' Group:    grp_order
            ' In Tab:   tab_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "tab_testing"
            ' Tab:   tab_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "tab_project"
            ' Tab:   tab_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_technology"
            ' Tab:   tab_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_formulation"
            ' Tab:   tab_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "grp_project"
            ' Tab:   tab_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        Case "btn_project"
            ' Tab:   tab_testing
            label = GetTranslation("ribbon_bar", control.ID, GetDbSetting("language"))
        
        
        Case Else
            label = "*getLabel*"

    End Select

End Sub

Sub GetScreentip(control As IRibbonControl, ByRef screentip)
    ' Callbackname in XML File "getScreentip"
    ' To set the property "screentip" to a Ribbon Control

    Select Case control.ID
        

        Case Else
            screentip = "*getScreentip*"

    End Select

End Sub

Sub GetSupertip(control As IRibbonControl, ByRef supertip)
    ' Callbackname in XML File "getSupertip"
    ' To set the property "supertip" to a Ribbon Control

    Select Case control.ID
        

        Case Else
            supertip = "*getSupertip*"

    End Select

End Sub

Sub GetDescription(control As IRibbonControl, ByRef description)
    ' Callbackname in XML File "getDescription"
    ' To set the property "description" to a Ribbon Control

    Select Case control.ID
        

        Case Else
            description = "*getDescription*"

    End Select

End Sub

Sub GetTitle(control As IRibbonControl, ByRef title)
    ' Callbackname in XML File "getTitle"
    ' To set the property "title" to a Ribbon MenuSeparator Control

    Select Case control.ID
        

        Case Else
            title = "*getTitle*"

    End Select

End Sub

'Button

Sub OnActionButton(control As IRibbonControl)
    ' Callbackname in XML File "onAction"

    ' Callback for event button click
    ' Callback fuer Button Click
    
    Select Case control.ID
        
        Case "btn_database"
            ' In Tab:   tab_setup
            ' In Group: grp_server
            DoCmd.OpenForm "dbsetup", acNormal, , , acFormEdit, acWindowNormal
        Case "btn_setup"
            ' In Tab:   tab_setup
            ' In Group: grp_config
            DoCmd.OpenForm "setup", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_customfield"
            ' In Tab:   tab_setup
            ' In Group: grp_config
            DoCmd.OpenForm "customfield", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_translation"
            ' In Tab:   tab_setup
            ' In Group: grp_config
            DoCmd.OpenForm "translation", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_user"
            ' In Tab:   tab_setup
            ' In Group: grp_config
            DoCmd.OpenForm "users", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_role"
            ' In Tab:   tab_setup
            ' In Group: grp_config
            DoCmd.OpenForm "role", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_mailqueue"
            ' In Tab:   tab_setup
            ' In Group: grp_config
            DoCmd.OpenForm "mailqueue", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_error"
            ' In Tab:   tab_setup
            ' In Group: grp_config
            DoCmd.OpenForm "errorlog", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_billing"
            ' In Tab:   tab_setup
            ' In Group: grp_config
            DoCmd.OpenForm "billing", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_customer"
            ' In Tab:   tab_catalogs
            ' In Group: grp_address
            DoCmd.OpenForm "customer", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_contact"
            ' In Tab:   tab_catalogs
            ' In Group: grp_address
            DoCmd.OpenForm "contact", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_manufacturer"
            ' In Tab:   tab_catalogs
            ' In Group: grp_address
            DoCmd.OpenForm "manufacturer", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_supplier"
            ' In Tab:   tab_catalogs
            ' In Group: grp_address
            DoCmd.OpenForm "supplier", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_smpmatrix"
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            DoCmd.OpenForm "smpmatrix", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_smptype"
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            DoCmd.OpenForm "smptype", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_smppoint"
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            DoCmd.OpenForm "smppoint", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_smpcondition"
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            DoCmd.OpenForm "smpcondition", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_smppreservation"
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            DoCmd.OpenForm "smppreservation", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_container"
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            DoCmd.OpenForm "smpcontainer", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_storage"
            ' In Tab:   tab_catalogs
            ' In Group: grp_sampling
            DoCmd.OpenForm "storage", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_instype"
            ' In Tab:   tab_catalogs
            ' In Group: grp_other
            DoCmd.OpenForm "instype", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_technique"
            ' In Tab:   tab_catalogs
            ' In Group: grp_other
            DoCmd.OpenForm "technique", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_priority"
            ' In Tab:   tab_catalogs
            ' In Group: grp_other
            DoCmd.OpenForm "priority", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_laboratory"
            ' In Tab:   tab_infrastructure
            ' In Group: grp_instance
            DoCmd.OpenForm "laboratory", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_department"
            ' In Tab:   tab_infrastructure
            ' In Group: grp_institution
            DoCmd.OpenForm "department", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_workplace"
            ' In Tab:   tab_infrastructure
            ' In Group: grp_institution
            DoCmd.OpenForm "workplace", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_instrument"
            ' In Tab:   tab_infrastructure
            ' In Group: grp_institution
            DoCmd.OpenForm "instrument", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_desktop"
            ' In Tab:   tab_testing
            ' In Group: grp_certificate
            DoCmd.OpenForm "desktop", acNormal, , , acFormReadOnly, acWindowNormal
        Case "btn_info"
            ' In Tab:   tab_testing
            ' In Group: grp_certificate
            DoCmd.OpenForm "_info", acNormal, , , acFormReadOnly, acWindowNormal
        Case "btn_certificate"
            ' In Tab:   tab_testing
            ' In Group: grp_certificate
            DoCmd.OpenForm "certificate", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_qualification"
            ' In Tab:   tab_testing
            ' In Group: grp_certificate
            DoCmd.OpenForm "qualification", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_method"
            ' In Tab:   tab_testing
            ' In Group: grp_testing
            DoCmd.OpenForm "method", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_analysis"
            ' In Tab:   tab_testing
            ' In Group: grp_testing
            DoCmd.OpenForm "analysis", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_profile"
            ' In Tab:   tab_testing
            ' In Group: grp_testing
            DoCmd.OpenForm "profile", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_template"
            ' In Tab:   tab_testing
            ' In Group: grp_testing
            DoCmd.OpenForm "template", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_workflow"
            ' In Tab:   tab_testing
            ' In Group: grp_order
            DoCmd.OpenForm "workflow", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_mdi"
            ' In Tab:   tab_testing
            ' In Group: grp_order
            useMDIMode
        Case "btn_request"
            ' In Tab:   tab_testing
            ' In Group: grp_order
            If DbProcedures.GetPermission("request").Read Then
                DoCmd.OpenForm "request", acFormDS, , , acFormEdit, acWindowNormal
            Else
                If DbProcedures.GetPermission("request_owner").Read Then DoCmd.OpenForm "request_owner", acFormDS, , , acFormEdit, acWindowNormal
            End If
        Case "btn_measurement"
            ' In Tab:   tab_testing
            ' In Group: grp_order
            DoCmd.OpenForm "view_measurement", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_batch"
            ' In Tab:   tab_testing
            ' In Group: grp_order
            DoCmd.OpenForm "batch", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_material"
            ' In Tab:   tab_testing
            ' In Group: grp_order
            DoCmd.OpenForm "material", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_service"
            ' In Tab:   tab_testing
            ' In Group: grp_order
            DoCmd.OpenForm "service", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_formulation"
            ' In Tab:   tab_testing
            ' In Group: grp_order
            DoCmd.OpenForm "formulation", acFormDS, , , acFormEdit, acWindowNormal
        Case "btn_project"
            ' In Tab:   tab_testing
            ' In Group: grp_order
            If DbProcedures.GetPermission("project").Read Then
                DoCmd.OpenForm "project", acFormDS, , , acFormEdit, acWindowNormal
            Else
                If DbProcedures.GetPermission("project_owner").Read Then DoCmd.OpenForm "project_owner", acFormDS, , , acFormEdit, acWindowNormal
            End If
      
        Case Else
            MsgBox "Button """ & control.ID & """ clicked" & vbCrLf & _
                           "Es wurde auf Button """ & control.ID & """ im Ribbon geklickt", _
                           vbInformation
    End Select

End Sub

'Command Button

Sub OnActionButtonHelp(control As IRibbonControl, ByRef CancelDefault)
    ' Callbackname in XML File Command "onAction"

    ' Callback for command event button click
    ' Callback fuer Command Button Click

    MsgBox "Button ""Help"" clicked" & vbCrLf & _
                           "Es wurde auf Button ""Hilfe"" geklickt", _
                           vbInformation
    CancelDefault = True

End Sub

'CheckBox

Sub OnActionCheckBox(control As IRibbonControl, _
                     pressed As Boolean)
    ' Callbackname in XML File "OnActionCheckBox"
    
    ' Callback for event checkbox click
    ' Callback fuer Checkbox Click

    Select Case control.ID
        
        
        Case Else
            MsgBox "The Value of the Checkbox """ & control.ID & """ is: " & pressed & vbCrLf & _
                   "Der Wert der Checkbox """ & control.ID & """ ist: " & pressed, _
                   vbInformation

    End Select

End Sub

Sub GetPressedCheckBox(control As IRibbonControl, _
                       ByRef bolReturn)
    ' Callbackname in XML File "GetPressedCheckBox"
    
    ' Callback for checkbox
    ' indicates how the control is displayed
    ' Callback fuer Checkbox wie das Control
    ' angezeigt werden soll

    Select Case control.ID
        

        Case Else
            If getTheValue(control.Tag, "DefaultValue") = "1" Then
                bolReturn = True
            Else
                bolReturn = False
            End If

    End Select

End Sub

'ToggleButton

Sub OnActionTglButton(control As IRibbonControl, _
                      pressed As Boolean)
                              
    ' Callbackname in XML File "onAction"
    
    ' Callback fuer einen Toggle Button Klick
    ' Callback for a Toggle Buttons click event

    Select Case control.ID
        

        Case Else
            MsgBox "The Value of the Toggle Button """ & control.ID & """ is: " & pressed & vbCrLf & _
                   "Der Wert der Toggle Button """ & control.ID & """ ist: " & pressed, _
                   vbInformation

    End Select

End Sub

Sub GetPressedTglButton(control As IRibbonControl, _
                        ByRef pressed)
    ' Callbackname in XML File "getPressed"

    ' Callback fuer ein Access ToogleButton Control wie dieser Angezeigt werden soll
    ' Callback for an Access ToogleButton Control. Indicates how the control is displayed

    Select Case control.ID
        

        Case Else
            If getTheValue(control.Tag, "DefaultValue") = "1" Then
                pressed = True
            Else
                pressed = False
            End If

    End Select

End Sub

'EditBox

Sub GetTextEditBox(control As IRibbonControl, _
                   ByRef strText)
    ' Callbackname in XML File "GetTextEditBox"
    
    ' Callback fuer EditBox welcher Wert in der
    ' EditBox eingetragen werden soll.
    ' Callback for an EditBox Control
    ' Indicates which value is to set to the control

    Select Case control.ID
        

        Case Else
            strText = getTheValue(control.Tag, "DefaultValue")
    
    End Select
    
End Sub

Sub OnChangeEditBox(control As IRibbonControl, _
                    strText As String)
    ' Callbackname in XML File "OnChangeEditBox"
    
    ' Callback Editbox: Rueckgabewert der Editbox
    ' Callback Editbox: Return value of the Editbox

    Select Case control.ID
        

        Case Else
            MsgBox "The Value of the EditBox """ & control.ID & """ is: " & strText & vbCrLf & _
                   "Der Wert der EditBox """ & control.ID & """ ist: " & strText, _
                   vbInformation

    End Select

End Sub

'DropDown

Sub OnActionDropDown(control As IRibbonControl, _
                     selectedId As String, _
                     selectedIndex As Integer)
    ' Callbackname in XML File "OnActionDropDown"
    
    ' Callback onAction (DropDown)
    
    Select Case control.ID
        

        Case Else
            Select Case selectedId
                Case Else
                    MsgBox "The selected ItemID of DropDown-Control """ & control.ID & """ is : """ & selectedId & """" & vbCrLf & _
                           "Die selektierte ItemID des DropDown-Control """ & control.ID & """ ist : """ & selectedId & """", _
                           vbInformation
            End Select
    End Select

End Sub


Sub GetSelectedItemIndexDropDown(control As IRibbonControl, _
                                 ByRef Index)
    ' Callbackname in XML File "GetSelectedItemIndexDropDown"
    
    ' Callback getSelectedItemIndex
    
    Dim varIndex As Variant
    varIndex = getTheValue(control.Tag, "DefaultValue")
    
    If IsNumeric(varIndex) Then
        Select Case control.ID
            

            Case Else
                Index = getTheValue(control.Tag, "DefaultValue")

        End Select

    End If

End Sub

'Gallery

Sub GetSelectedItemIndexGallery(control As IRibbonControl, _
                                   ByRef Index)
    ' Callbackname in XML File "GetSelectedItemIndexGallery"
    
    ' Callback GetSelectedItemIndexGallery
    
    Dim varIndex As Variant
    varIndex = getTheValue(control.Tag, "DefaultValue")
    
    If IsNumeric(varIndex) Then
        Select Case control.ID
            

            Case Else
                Index = varIndex

        End Select

    End If

End Sub

Sub OnActionGallery(control As IRibbonControl, _
                     selectedId As String, _
                     selectedIndex As Integer)
    ' Callbackname in XML File "OnActionGallery"
    
    ' Callback onAction (Gallery)
    
    Select Case control.ID
        

        Case Else
            Select Case selectedId
                Case Else
                    MsgBox "The selected ItemID of Gallery-Control """ & control.ID & """ is : """ & selectedId & """" & vbCrLf & _
                           "Die selektierte ItemID des Gallery-Control """ & control.ID & """ ist : """ & selectedId & """", _
                           vbInformation
            End Select
    End Select

End Sub

'Combobox

Sub GetTextComboBox(control As IRibbonControl, _
                      ByRef strText)

    ' Callbackname im XML File "GetTextComboBox"
    
    ' Callback getText (Combobox)
                           
    Select Case control.ID
        

        Case Else
            strText = getTheValue(control.Tag, "DefaultValue")
    End Select

End Sub


Sub OnChangeComboBox(control As IRibbonControl, _
                               strText As String)
                           
    ' Callbackname im XML File "OnChangeCombobox"
    
    ' Callback onChange (Combobox)
   
    Select Case control.ID
        

        Case Else
            MsgBox "The selected Item of Combobox-Control """ & control.ID & """ is : """ & strText & """" & vbCrLf & _
                   "Das selektierte Item des Combobox-Control """ & control.ID & """ ist : """ & strText & """", _
                   vbInformation
    End Select

End Sub

' DynamicMenu

Sub GetContent(control As IRibbonControl, _
               ByRef XMLString)

    ' Sample for a Ribbon XML "getContent" Callback
    ' See also http://www.accessribbon.de/en/index.php?Access_-_Ribbons:Callbacks:dynamicMenu_-_getContent
    '     and: http://www.accessribbon.de/en/index.php?Access_-_Ribbons:Ribbon_XML___Controls:Dynamic_Menu

    ' Beispiel fuer einen Ribbon XML - "getContent" Callback
    ' Siehe auch: http://www.accessribbon.de/index.php?Access_-_Ribbons:Callbacks:dynamicMenu_-_getContent
    '       und : http://www.accessribbon.de/?Access_-_Ribbons:Ribbon_XML___Controls:Dynamic_Menu

    Select Case control.ID
        

        Case Else
            XMLString = getXMLForDynamicMenu()
    End Select
 
End Sub


' Helper Function
' Hilfsfunktionen

Public Function getXMLForDynamicMenu() As String
    
    ' Creates a XML String for DynamicMenu CallBack - getContent
    
    ' Erstellt den Inhalt fuer das DynamicMenu im Callback getContent
    
    Dim lngDummy    As Long
    Dim strDummy    As String
    Dim strContent  As String
    
    Dim Items(4) As ItemsVal
    Items(0).ID = "btnDy1"
    Items(0).label = "Item 1"
    Items(0).imageMso = "_1"
    Items(1).ID = "btnDy2"
    Items(1).label = "Item 2"
    Items(1).imageMso = "_2"
    Items(2).ID = "btnDy3"
    Items(2).label = "Item 3"
    Items(2).imageMso = "_3"
    Items(3).ID = "btnDy4"
    Items(3).label = "Item 4"
    Items(3).imageMso = "_4"
    Items(4).ID = "btnDy5"
    Items(4).label = "Item 5"
    Items(4).imageMso = "_5"
    
    strDummy = "<menu xmlns=""http://schemas.microsoft.com/office/2009/07/customui"">" & vbCrLf
    
        For lngDummy = LBound(Items) To UBound(Items)
            strContent = strContent & _
                "<button id=""" & Items(lngDummy).ID & """" & _
                " label=""" & Items(lngDummy).label & """" & _
                " imageMso=""" & Items(lngDummy).imageMso & """" & _
                " onAction=""OnActionButton""/>" & vbCrLf
        Next
 

    strDummy = strDummy & strContent & "</menu>"
    getXMLForDynamicMenu = strDummy

End Function

Public Function getTheValue(strTag As String, strvalue As String) As String
    ' *************************************************************
    ' Created from     : Avenius
    ' Parameter        : Input String, SuchValue String
    ' Date created     : 05.01.2008
    '
    ' Sample:
    ' getTheValue("DefaultValue:=Test;Enabled:=0;Visible:=1", "DefaultValue")
    ' Return           : "Test"
    ' *************************************************************
      
   On Error Resume Next
      
   Dim workTb()     As String
   Dim Ele()        As String
   Dim myVariabs()  As String
   Dim i            As Integer

      workTb = Split(strTag, ";")
      
      ReDim myVariabs(LBound(workTb) To UBound(workTb), 0 To 1)
      For i = LBound(workTb) To UBound(workTb)
         Ele = Split(workTb(i), ":=")
         myVariabs(i, 0) = Ele(0)
         If UBound(Ele) = 1 Then
            myVariabs(i, 1) = Ele(1)
         End If
      Next
      
      For i = LBound(myVariabs) To UBound(myVariabs)
         If strvalue = myVariabs(i, 0) Then
            getTheValue = myVariabs(i, 1)
         End If
      Next
      
End Function

Public Function getAppPath() As String
    Dim strDummy As String
    strDummy = CurrentProject.Path
    If Right(strDummy, 1) <> "\" Then strDummy = strDummy & "\"
    getAppPath = strDummy
End Function

Public Function getIconFromTable(strFileName As String) As Picture
'*****************************************************************************
'Funktion 'getIconFromTable' holt ein Bild aus der Binaer-Tabelle und gibt
' ein Picture Objekt zurueck
' strFilename ist Bildobjekt welches in der Datei 'tblBinary" vorhanden ist.
'*****************************************************************************

Dim lSize As Long
Dim arrBin() As Byte
Dim rs As DAO.Recordset
 
    On Error GoTo Errr
 
    Set rs = DBEngine(0)(0).OpenRecordset("tblBinary", dbOpenDynaset)
    rs.FindFirst "[FileName]='" & strFileName & "'"
    If rs.NoMatch Then
        Set getIconFromTable = Nothing
    Else
        lSize = rs.Fields("binary").FieldSize
        ReDim arrBin(lSize)
        arrBin = rs.Fields("binary").GetChunk(0, lSize)
        Set getIconFromTable = ArrayToPicture(arrBin)
    End If
    rs.Close
 
fExit:
    Reset
    Erase arrBin
    Set rs = Nothing
    Exit Function
Errr:
    Resume fExit
End Function

Public Function getPic(strFullPath As String) As String
    Dim strResult As String
    
    If InStrRev(strFullPath, "\") > 0 Then
        strResult = Mid(strFullPath, InStrRev(strFullPath, "\") + 1)
    Else
        strResult = ""
    End If
   
    getPic = strResult
End Function


