...
'-------------------------------------------------------------------------------
    ...
    With fDialog
      ...
      If .show = True Then
        ...
        If FileLen(GetFileName(fDialog.SelectedItems(1))) > GetMaxFileSize() Then
            MsgBox GetTranslation("Dialogs", "max_filesize_exceeded", GetDbSetting("language")), vbInformation, GetTranslation("msgbox", "vbInformation", GetDbSetting("language"))
            OpenFileDialog = ""
        End If
      Else
        OpenFileDialog = ""
...
'Date:              2024 August
'Purpose:           Shows a Folder Open dialog
'Out:               File path
'-------------------------------------------------------------------------------
    'Requires reference to Microsoft Office 11.0 Object Library.
    Dim fDialog As Office.FileDialog
    ...
    With fDialog
      ...
      .Filters.Clear
      If .show = True Then
        OpenFolderDialog = fDialog.SelectedItems(1)
        If FileLen(GetFileName(fDialog.SelectedItems(1))) > GetMaxFileSize() Then
            MsgBox GetTranslation("Dialogs", "max_filesize_exceeded", GetDbSetting("language")), vbInformation, GetTranslation("msgbox", "vbInformation", GetDbSetting("language"))
            OpenFolderDialog = ""
        End If
...
Public Function SelectDialog(ByVal templateId As String) As Variant
'-------------------------------------------------------------------------------
...
'-------------------------------------------------------------------------------
    Dim rpt As AccessObject, db As Object, i As Long, s As Variant
    ...
    For Each rpt In db.AllReports
        If InStr(1, rpt.name, templateId, vbTextCompare) = 1 Then CurrentDb.Execute "INSERT INTO keyvalue (key, value_txt) VALUES(" & i & ",'" & rpt.name & "')"
        i = i + 1
    Next rpt
...
On Error GoTo Catch_Error
    ...
    Set db = CurrentDb()
    Set rs = db.OpenRecordset("SELECT TOP 1 upload_max_byte FROM setup")
